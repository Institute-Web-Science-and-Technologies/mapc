...
public class Connection implements Runnable{
	...
	private class Reader implements Runnable {
		...
		private void runLineReader() {
			...
			try {
				...
				// This should only happen when the socket is closed by the user... so this is no real failure.
				outerRun = false;	
			} catch (IOException e) {
	...
	private void init(int type) {
		...
		if(type != LINE_BASED_CONNECTION && type != OBJECT_BASED_CONNECTION)
			throw new IllegalArgumentException("type must be LINE_ or OBJECT_ BASE_CONNECTION");
		mType = type;
