...
@becomeACoach
+!choseZoningRole:
    bestZone(_, _)[source(self)]
    & broadcastAgentList(BroadcastList) //TODO replace with internal action which returns the list of idle zoners?
    & .my_name(MyName)
...
+!isZonePossible:
    bestZone(_, _)
    &  ia.getAvailableZoner(AvailableZoners)
    & .length(UsedNodes) -1 >  AvailableZoners
    <- !cancelledZoneBuilding.

+!isZonePossible
    <- ia.builtZone(CentreNode, ZoneSize).
...
//    isCoach(true)
//    & bestZone(_, _, CentreNode, UsedNodes, ClosestAgents)
...
+!toldMinionsTheirPosition:
    ...
    <- for (.range(ControlVariable, 0, MappingLength - 1)) {
       ...
       // if the zone the coach plus minions want to go to needs less agents than the coach has under his control
       // the redundant agents are told to find aother zone
...
//    isCoach(true)
//    & not bestZone(_, _, CentreNode, _, _)
//    <- -negativeZoneReply(CentreNode)[source(Sender)].
...
//    & bestZone(_, _, CentreNode, UsedNodes, _)
...
+!asyncForeignBestZone(Value, CentreNode, UsedNodes, ClosestAgents)[source(Sender)]:
	isCoach(true)
	& bestZone(BestZoneValue, BestZoneCentreNode)
	...
	<- //?plannedZoneTimeInSteps(Steps);
       ...
       if (Value > BestZoneValue & UsedNodes <= BestZoneUsedNodes+1){
       	-+bestZone(Value, CentreNode);
