...
Our first approach here was the cartographer agent we illustrated in \autoref{alg:map_cartographer}.
It was an additional agent in the background that was sent all the information about the map which all 28 agents perceived.
With that cartographer agent the amount of communication was reduced and agents could act like intended, because
Agents would then just sent their percepts to the cartographer agent and did not have to deal with incoming messages of the other 27 agents.
The drawback of this approach revealed when it came to querying the cartographer agent for information.
Agents needed to do this frequently for instance when an they wanted to know if a vertex was already surveyed or how a given vertex could be reached.
...
% TODO: this section is all negative. We should have some positive things as well!
