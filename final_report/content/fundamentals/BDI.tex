...
The former process is known as \emph{deliberation}, the latter as \emph{means-ends reasoning}~\cite{Gerhard_MultiSystem_1999}\todo{\textbf{yuansun1990}: Although you mention it here, you don't explain means-end reasoning. But you have to do so.}.
...
Beliefs represent the informational state of the agent\todo{\textbf{@yuansun1990}: YOU ARE QUOTING WIKIPEDIA WITHOUT ANY CITATIONS. THIS IS UNACCEPTABLE!} and are updated appropriately after each sensing action.
...
A revision operator * then maps a belief state $K$ and a formula $P$ to a revised belief state $K$ * $P$~\cite{James_revise_2011}\todo{\textbf{@yuansun1990}: For me, it is unclear, why this is needed or what will change. Is $K_n:=K_{n-1}*P_i$? How does the revision function change?}.
