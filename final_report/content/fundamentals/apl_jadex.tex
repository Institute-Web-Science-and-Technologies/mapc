...
Nowadays a couple of agent frameworks are available for developing multi-agent applications \cite{Mangina}. % TODO maybe this fits better in the general part (parent section) – btw, this is nearly a quote to the original's papers abstract…
...
% TODO @manuelmittler I haven't read all the related papers so I am not sure which information is from what source. Please add citations accordingly. You can look at my other APL sections to see how I handled it by explaining that everything following is from $SOURCE_1 except when noted differently.
% TODO @manuelmittler you seem to like to use brackets for additional information. Please refer to the guidelines which advise against using them.
...
% TODO: @manuelmittler what are ``agent-theories''?
...
\begin{figure}
  ...
  % TODO @manuelmittler: what is the source of this graphic?
	\caption{Jadex Infrastrucuture}
	\label{fig1}
\end{figure}
% TODO @manuelmittler as far as I can see, you never reference or explain this figure. What is its purpose then? Either remove it or embed it into a textual description.
...
% TODO @manuelmittler what does it mean to control its own lifecycle?
The communication happens by exchanging asynchronous messages via an \emph{agent communication language} (short: ACL). % TODO @manuelmittler is ACL the language or is this just any communication language for agents? I guess, you mean FIPA ACL which are a special form of ACL. If so, mention and reference FIPA ACL and explain in one or two sentences what it is and why it is special. Else, there is no reason to treat it as a special term and giving it an abbreviation that isn't used later on.
...
\begin{figure}
	...
	\label{fig2}
\end{figure}
\autoref{fig2} depicts an abstract view on a Jadex agent. Every agent may receive messages which trigger internal events that can change his internal knowledge, plans or goals. Interactions with the outside like the environment or other agents happens through the sending of messages. % TODO @manuelmittler I removed the quote as I did not find it to be necessary here. Am I correct that modifications in the environment are also done through messages?
In more detail, beliefs are single facts stored as Java objects which represent the knowledge of an agent. % TODO @manuelmittler should I know, what a fact is? Because I honestly don't..
They are stored as key-value pairs.
The advantage of storing information as facts is that the programmer has a central place for the knowledge and can query the agent's beliefs. % TODO @manuelmittler or do you mean that there is an advantage by storing them as Java Objects?
