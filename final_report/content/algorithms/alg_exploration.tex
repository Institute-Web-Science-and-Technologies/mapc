...
Every agent gets beliefs about its local environment quickly by receiving percepts from the server. % TODO @adaudrich actually, this is not ``of course''. Up to here, the reader does not know how percepts are getting through to the agent or from where they are coming – as long as I am not overlooking something in an earlier section that is. This is just being illustrated in the later sections.
...
Finally, this chapter concludes with a description of the second approach we used and finally stuck to in \autoref{alg:map_javamap}. % I'm unsure whether ``sticking to something'' is colloquial or at least too informal.
...
\autoref{fig:map:com2} shows the second approach we used and \autoref{fig:map:comp2} the corresponding distribution model\todo{\textbf{@adaudrich}: update \autoref{fig:map:com2} as we discussed in our private Hangouts chat: all percept transmission to the node agents happened through the cartographer agent because this was the only one who could ensure that the node agent existed. The querying happened directly although the depiction is simplified. A commit that shows this well, is e.g. e457e8bd4b59b3bb960db4ffcd84f407f18f087f}.
\begin{figure}
  \centering
...
DV is a routing protocol based on the Bellman-Ford algorithm and normally has its use in packet-switched networks\todo{reference}.
...
Because the Bellman-Ford algorithm has no loop detection\todo{reference}, we had to implement some kind of break condition for the algorithm.
...
At all times, the necessary map information was first received from the Java EIS Environment module\todo{I don't think, we mention this earlier}.
...
Our change here was to add a filter to this module which would only transmit new information and would send it directly to the respective note agents\todo{\textbf{@adaudrich}: is this true? I'm looking at e107568cd43869b6be119079ecc9bac80bd4a094}.
...
We decided to implement the whole map module in Java\todo{am I mistaken or doesn't JavaMap take care of two two agents not running towards the same node during exploration or do they negotiate? I think, we never mention this explicitly here, but we should.}.
