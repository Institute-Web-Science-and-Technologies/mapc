...
Our initial approach was to install one additional, omniscient agent which we called the \emph{cartographer agent}.
It was a separate agent existing in the background which was independent from our 28 agents of the simulation.
The cartographer agent represented a map and had the purpose to calculate shortest paths between given vertices as well as storing map information.
The map information included data like traversing costs, edges between vertices and vertex values.
...
By distributing the information from one cartographer agent to a huge network of node agents we also distributed the load from one agent, the cartographer agent, between the respective node agents.
