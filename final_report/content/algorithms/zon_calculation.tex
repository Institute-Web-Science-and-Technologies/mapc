...
\begin{itemize}
  ...
  \item Besides the centre vertex itself, agents should only be placed on the centre vertex's two-hop neighbours.
    ...
    Later, we will illustrate that there are rare cases in which agents must be placed on one-hop neighbours as well\todo{\textbf{@msewell}: I added this because $\exists A_v \nsubseteq v \cup V_{v}^{2}$ and the $A_v$ \emph{should} somewhat implied that no one-hop-vertices would be included.}.
...
\begin{figure}
  ...
  \subcaptionbox{This zone was calculated for a centre vertex that only has a degree of 1, i.e.\ that is a leaf vertex.
    Generally, it is preferable to place an agent on the articulation point that leads to a leaf vertex rather than the leaf vertex itself, as this would establish at least a zone of equal size, and possibly larger\todo{\textbf{@msewell}: I replaced ``cut vertex'' with ``articulation point'' for consistency. If cut vertex would be more correct for some reason, we may undo this.}.
  ...
  \subcaptionbox{A zone where the centre vertex has a degree of 7, and the zone uses a total of 9 agents.
           ...
           \caption{Four examples of zones calculated by the heuristic algorithm described in \autoref{alg:zon_calculation}\todo{\textbf{@msewell}: it would be nice if all those graphics had the same image aspect ratio.}
...
The steps of the algorithm are easiest illustrated graphically, as in \autoref{fig:coloring}\todo{should we put this into the appendix? Currently it appears around two pages later}.
\begin{definition}
...
\begin{figure}
  ...
  \caption{The zone calculation algorithm shown in four steps\todo{we might should push it into the appendix. In my most recent compilation, it appears five pages after its section.}
