...
\begin{itemize}
  ...
  \item Besides the centre vertex itself, agents should only be placed on the centre vertex's two-hop neighbours.
    ...
    Later, we will illustrate that there are rare cases in which agents must be placed on one-hop neighbours as well\todo{I added this because $\exists A_v \nsubseteq v \cup V_{v}^{2}$ and the $A_v$ \emph{should} somewhat implied that no one-hop-vertices would be included.}.
...
\begin{figure}
  ...
  \subcaptionbox{This zone was calculated for a centre vertex that only has a degree of 1, i.e.\ that is a leaf vertex.
                 Generally, it is preferable to place an agent on the articulation point that leads to a leaf vertex rather than the leaf vertex itself, as this would establish at least a zone of equal size, and possibly larger\todo{I replaced ``cut vertex'' with ``articulation point'' for consistency. If cut vertex would be more correct for some reason, we may undo this.}.
  ...
  \caption{Four examples of zones calculated by the heuristic algorithm described in \autoref{alg:zon_calculation}\todo{it would be nice if all those graphics had the same image aspect ratio.}
