...
\begin{figure}
  ...
  \subcaptionbox{The initial step.
                 All vertices within the centre vertex's two-hop neighbourhood are considered optional agent positions at this point.
                 Note that this also includes those vertices that are at distance 1 from the centre vertex, even though we ideally only want to place agents on vertices that are 2 edges away.
                 There are some possible fringe cases where it is impossible to fulfill the criteria of having the entire one-hop neighbourhood in the zone by only placing agents on the two-hop vertices, however, and so all the vertices in the one-hop neighbourhood must initially be considered for agent placement.
                 \label{fig:coloring1}}[.49\linewidth]{\includegraphics[width=.49\linewidth]{images/coloring1.png}}
  \subcaptionbox{The algorithm proceeds to add all two-hop vertices that are connected to two or more one-hop vertices to the list of definitive agent positions, denoted by the green colour.
                 Afterwards, those two-hop vertices connected either directly to a \enquote{green} vertex or indirectly connected to a \enquote{green} vertex through a single one-hop vertex are removed from the list of possible agent positions (yellow).
                 The reasoning behind this is that those one-hop vertices adjacent to the now green vertices are already definitely included in the zone, and the removed vertices do not contribute towards that goal.
  ...
  \subcaptionbox{In the next step, \enquote{bridges} are discovered in the list of remaining yellow two-hop neighbours.
                 ...
                 If such a bridge exists in the list of remaining two-hop vertices, all three vertices can be included in the zone by placing an agent on either end of the chain and leaving out the in-between vertex.
  ...
  \subcaptionbox{In the last step, the algorithm checks if all one-hop neighbours are connected to an agent position (green vertices).
                 This is most frequently the case, as in this example, but not always.
                 If a remaining, unconnected one-hop is found, we check if it is connected to one or more \enquote{yellow} two-hop vertices.
                 If that is the case, we choose the neighbouring two-hop with the highest vertex value and add it to the list of agent positions.
                 If no such two-hop vertex is found, we add the unconnected one-hop vertex to the list of agent positions---this is the only case where a one-hop vertex can be added to the list of agent positions.
                 Finally, we include the center vertex in the list of agent positions and colour all remaining yellow one-hops red.
                 Any yellow vertices that remain are saved as possible agent positions that could be used to extend the zone by otherwise idle agents, but unlike the green vertices are not required to establish the initial smallest zone that we calculated.
